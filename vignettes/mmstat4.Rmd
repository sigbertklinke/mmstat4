...
```{r}
library("mmstat4")
ghget(test="https://github.com/sigbertklinke/mmstat4.dummy/archive/refs/heads/main.zip") # make it the current repo and installs it if necessary
ghrepos()
```
...
and answer the question  `Download and install repository to 'xxx'? (Yes/no/cancel)` with

* `y` to install it permamently to `XXX`, or
* `n` to install it temporarily.

Note: If a repository was installed permanently and you switch back temporarily then the downloaded files will not be deleted.
...
| `hu.stat`        | 40 MB         | `https://github.com/sigbertklinke/mmstat4.data/archive/refs/heads/main.zip`   |
| `hu.data`        | 20 MB         | `https://github.com/sigbertklinke/mmstat4.stat/archive/refs/heads/main.zip`   |
"
cat(tabl) 
```
