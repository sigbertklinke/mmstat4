mmstat.getDataset <- function (file) {
...
mmstat.getVar <- function (dataname, varname, vartype='numvars') {
  deftype <- c('numvars')
  type   <- pmatch(vartype, deftype)
  if (is.na(type)) stop(paste0('Unknown variable type: ', vartype))
  type   <- deftype[type]
  inpvar <- default(varname, mmstat$dataset[[dataname]][[type]][1])
  values <- na.omit(with(mmstat$dataset[[dataname]]$data, get(inpvar)))
...
default <- function (var, def) { if (is.null(var)) def else var }
